...
def main() -> int:
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            page.click("#submit-case")
            page.wait_for_timeout(700)
            q1 = page.locator("#cases tbody tr", has_text="E2E-Q1")
            ...
            page.click('button[data-case-tab="list"]')
            page.wait_for_timeout(400)
            results.append(("面板核准後案件狀態為已核准", "已核准" in page.locator("#cases tbody tr", has_text="E2E-Q1").inner_text()))

            # 3.91) 追溯鏈：案件列「追溯鏈」按鈕 → 整條控管鏈(含預算/專案，先前漏接、這次補上)
            row_wiz = page.locator("#cases tbody tr", has_text="E2E-WIZ")
            ...
            row = page.locator("#cases tbody tr", has_text="E2E-001")
            ...
            page.wait_for_timeout(500)
            row4 = page.locator("#cases tbody tr", has_text="E2E-001")
            row4.get_by_role("button", name="核准").click()
            page.wait_for_timeout(600)
            results.append(("另一助理核准後狀態為已核准", "已核准" in page.locator("#cases tbody tr", has_text="E2E-001").inner_text()))
